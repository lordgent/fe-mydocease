  ...
  <div class="min-h-screen bg-white flex flex-col font-sans antialiased text-gray-900 overflow-x-hidden">
    ...
    <section id="tools" class="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-24">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-16 gap-6">
        ...
        <p
          class="text-gray-600 font-medium text-sm max-w-xs border-l-4 border-gray-800 pl-4 md:text-right md:border-l-0 md:border-r-4 md:pr-4 md:ml-auto">
    ...
    <section class="bg-gray-800 py-20 md:py-28 text-white">
      <div class="max-w-6xl mx-auto px-6">
        <h2
          class="text-[10px] md:text-xs font-black uppercase tracking-[0.5em] text-gray-400 mb-16 md:mb-24 text-center">
          How it works</h2>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-16">
          <div v-for="(step, index) in steps" :key="index" class="relative group">
            <div
              class="text-[70px] md:text-[100px] font-black text-white/10 absolute -top-12 md:-top-16 -left-2 md:-left-4 leading-none select-none group-hover:text-white/20 transition-colors">
              {{ index + 1 }}
            </div>
            <h4 class="text-base md:text-lg font-black uppercase tracking-widest mb-3 md:mb-4 relative z-10">{{
              step.title }}</h4>
    ...
    <SubscriptionList :show="isModalOpen" bgColor="bg-gray-900" @close="isModalOpen = false"
      @select="handlePlanSelection" :allowTrial="true" />
  </div>
</template>
...
const tools = [
  ...
  {
    title: 'Protect PDF',
    desc: 'Secure your documents with a password to prevent unauthorized access.',
    icon: '/icons/password-pdf.png',
    path: '/editor/security-pdf'
  },
